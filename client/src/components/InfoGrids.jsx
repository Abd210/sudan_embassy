...
function Hours() {
  ...
  const [hours, setHours] = useState({ monThu: '9:00 AM - 4:00 PM', fri: '9:00 AM - 1:00 PM' })
  useEffect(()=>{ fetch('http://localhost:3000/api/settings').then(r=>r.json()).then(s=>{ if (s?.hours) setHours(s.hours) }) },[])
  return (
    <div className="rounded-lg shadow-sm p-6 bg-gradient-to-br from-sudan-green to-[#0a6b47] text-white">
      ...
      <ul className="space-y-2">
        <li className="flex justify-between"><span>{t('contact.mon_thu')}</span><span>{hours.monThu}</span></li>
        <li className="flex justify-between"><span>{t('contact.fri')}</span><span>{hours.fri}</span></li>
        <li className="flex justify-between"><span>{t('contact.sat_sun')}</span><span>{t('contact.closed')}</span></li>
      </ul>
...
function Emergency() {
  const { t } = useTranslation()
  const [emergency, setEmergency] = useState({ phone: '+40 722 123 456', note: 'This number is for genuine emergencies only' })
  useEffect(()=>{ fetch('http://localhost:3000/api/settings').then(r=>r.json()).then(s=>{ if (s?.emergency) setEmergency(s.emergency) }) },[])
  return (
