...
export default function Services({ limit, carousel = false }) {
  ...
  useEffect(()=>{
    ...
    fetch(`http://localhost:3000/api/consular-services?lang=${encodeURIComponent(lang||'')}`).then(r=>r.json()).then((list)=>{
    ...
    return (
      <section id="services-section" data-aos="fade-up">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold text-sudan-black sudan-section-title">{t('services.title')}</h2>
          <a href="/consular-services" className="text-sudan-blue hover:underline">{t('common.view_all')}</a>
        </div>
        <div className="overflow-x-auto -mx-4 px-4 snap-x">
          <div className="grid grid-flow-col auto-cols-[minmax(300px,1fr)] gap-4 md:gap-6 mb-4">
            {list.map((s, idx) => (
              <div key={s.title} className="min-w-[300px] snap-start" data-aos="fade-up" data-aos-delay={idx*80}>
                <div className="card">
                  <ServiceCard {...s} />
                </div>
              </div>
  ...
  return (
    <section id="services-section" data-aos="fade-up">
      <h2 className="text-2xl font-bold text-sudan-black mb-4 sudan-section-title">{t('services.title')}</h2>
